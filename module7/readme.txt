...
Analyseur de PDF: V.0.9
	* Parsing du PDF
	* Analyse anti-malware
	* Evaluation (coefficient de suspicion)

--------------------------------------------------------------------------------------
Usage : ./uhuruPDFAnalyzer [file_path]

To scan a directory:
> bash scandir.sh [directory_path] [log_file_name]

NB: The directory scan produces a file "stats.txt" containing scan evaluation summary.
...
- Amélioration des filtres actuels (FlateDecode, CCITTFaxDecode, ASCII85Decode).
- Implémentation de nouveaux Filtres (DTCDecode, RunLengthDecode, JBIGDecode, etc.)
- XDP format parsing
...
- Amélioration de l'algo de la fonction "unknownPatternRepetition" (gain de temps).
